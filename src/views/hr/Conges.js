      ...
      <main style={{ minHeight: "100vh", background: "#f8fafc" }}>
        ...
        <div style={{ background: "linear-gradient(135deg,#0f172a 0%,#1e3a5f 60%,#1e293b 100%)", padding: "64px 24px 80px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ maxWidth: 900, margin: "0 auto", position: "relative" }}>
            ...
            <h1 style={{ fontSize: "clamp(24px,4vw,40px)", fontWeight: 800, color: "#fff", margin: "0 0 8px" }}>Demandes de Congé</h1>
            <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 14, margin: 0 }}>{conges.length} demande{conges.length !== 1 ? "s" : ""}</p>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 900, margin: "-32px auto 0", padding: "0 24px 60px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 14, marginBottom: 24 }}>
            {[
              ...
              <div key={s.label} style={{ background: "#fff", borderRadius: 16, padding: "20px 24px", boxShadow: "0 2px 16px rgba(30,60,120,0.06)", borderTop: `3px solid ${s.color}`, display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div>
                  <div style={{ fontSize: 24, fontWeight: 800, color: "#1a2340", lineHeight: 1 }}>{s.value}</div>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
            {[{ key: "ALL", label: "Toutes" }, { key: "EN_ATTENTE", label: "En attente" }, { key: "APPROUVE", label: "Approuvées" }, { key: "REFUSE", label: "Refusées" }].map(f => (
              <button key={f.key} onClick={() => setFilter(f.key)} style={{ padding: "8px 18px", borderRadius: 20, border: filter === f.key ? "none" : "1.5px solid #e2e8f0", background: filter === f.key ? "#1a2340" : "#fff", color: filter === f.key ? "#fff" : "#64748b", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
          ...
          {loading ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {filtered.map(c => {
                ...
                return (
                  <div key={c._id} style={{ background: "#fff", borderRadius: 16, padding: "20px 24px", boxShadow: "0 2px 16px rgba(30,60,120,0.06)", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap", borderLeft: `3px solid ${statut.color}` }}>
