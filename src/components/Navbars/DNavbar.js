      ...
      <nav style={{
        ...
        <Link to="/admin/dashboard" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
          ...
          <span style={{
            ...
            background: "rgba(245,158,11,0.12)", padding: "2px 8px", borderRadius: 4,
          }}>Admin</span>
        </Link>
  ...
  textDecoration: "none", cursor: "pointer",
      ...
      {open && (
        <div style={{ position: "fixed", top: 64, left: 0, right: 0, zIndex: 999, background: "#1e293b", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "12px 16px", display: "flex", flexDirection: "column", gap: 4 }}>
          {links.map(({ to, label, icon: Icon }) => (
            <Link key={to} to={to} onClick={() => setOpen(false)} style={{ padding: "10px 14px", borderRadius: 8, fontSize: 14, fontWeight: 600, color: isActive(to) ? "#fff" : "rgba(255,255,255,0.6)", background: isActive(to) ? "rgba(245,158,11,0.15)" : "transparent", textDecoration: "none", display: "flex", alignItems: "center", gap: 10 }}>
              <Icon size={16} /> {label}
            </Link>
          ))}
